...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for start in tqdm(range(0, len(users), batch_size), desc="eval", leave=False):
            ...
            ranklist = np.argpartition(-scores, kth=k - 1, axis=1)[:, :k]
            for i, u in enumerate(batch_users):
                gt = eval_data[u]
    ...
    for epoch in range(1, int(cfg["epochs"]) + 1):
        ...
        for _ in pbar:
            users, pos_items, neg_items = sample_bpr_batch(
                user_pos, data["num_items"], int(cfg["batch_size"])
            )
        ...
        print(f"Epoch {epoch}: Recall@{cfg['eval_k']}={metrics['recall']:.4f}, NDCG@{cfg['eval_k']}={metrics['ndcg']:.4f}")
