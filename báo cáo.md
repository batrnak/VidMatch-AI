...
### 5.2. Kết quả BPR-MF
BPR-MF tối ưu trực tiếp bài toán xếp hạng Top-N bằng hàm mất mát BPR Loss. Quá trình kiểm định được thực hiện tại mỗi epoch trên tập Validation để tìm ra checkpoint tốt nhất. Sau đó, checkpoint tốt nhất được đánh giá trên tập Test.
...
LightGCN tối ưu trực tiếp bài toán xếp hạng qua BPR Loss, được đánh giá trên tập Validation mỗi 10 epoch và dừng sớm khi hiệu năng không cải thiện sau 10 lần đánh giá liên tiếp.
...
**Phân tích kết quả:**
* **LightGCN vượt trội hơn BPR-MF** trên cả hai chỉ số xếp hạng: Recall@20 (**0.1384** so với **0.1209**) và NDCG@20 (**0.0540** so với **0.0483**).
* **Nguyên nhân học thuật:** BPR-MF là một mô hình tuyến tính phân rã ma trận, chỉ học trực tiếp các tương tác bậc 1 trực tiếp. Trong khi đó, **LightGCN** khai thác cấu trúc đồ thị lưỡng phân bằng cách thực hiện lan truyền đặc trưng qua nhiều lớp (3 lớp). Cơ chế này giúp thu thập các tín hiệu cộng tác đa cấp (multi-hop collaborative filtering), giúp biểu diễn nhúng của người dùng và phim phản ánh mối liên kết sâu hơn trong cộng đồng, từ đó giải quyết hiệu quả vấn đề ma trận thưa thớt (sparsity = 95.53%) của MovieLens 1M.
