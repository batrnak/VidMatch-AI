...
### 5.2. Kết quả BPR-MF
BPR-MF tối ưu trực tiếp bài toán xếp hạng Top-N bằng hàm mất mát BPR Loss. Quá trình kiểm định được thực hiện tại mỗi epoch: mô hình tính điểm tương thích $W_u^T X_i + b_i$ cho toàn bộ phim chưa xem, loại bỏ phim đã xem, rồi lọc ra Top 20 đề xuất.

* **Recall@20 = 0.1238** | **NDCG@20 = 0.0486** (Epoch 30)

![MF Metrics](checkpoints/mf_metrics_k20.png)
*Hình 5: Recall@20 và NDCG@20 của BPR-MF qua các epoch.*
...
*Hình 7: So sánh Recall@20 và NDCG@20 giữa BPR-MF và LightGCN.*

| Mô hình | Recall@20 | NDCG@20 |
| :--- | :--- | :--- |
| **BPR-MF** | **0.1238** | **0.0486** |
| LightGCN | 0.0833 | 0.0338 |

**Phân tích:**
* Trái ngược với kết quả của mô hình MF tối ưu MSE trước đây, mô hình **BPR-MF** sau khi tối ưu trực tiếp thứ tự xếp hạng bằng BPR Loss đã vượt trội hơn LightGCN ở cả hai chỉ số: Recall@20 (**0.1238** so với **0.0833**) và NDCG@20 (**0.0486** so với **0.0338**).
* Nguyên nhân cốt lõi: Trên tập dữ liệu MovieLens 1M, BPR-MF là một mô hình phân rã ma trận tuyến tính hiệu năng cao, ít bị hiện tượng lan truyền quá mức (oversmoothing) gây nhiễu embedding khi số tầng của GNN tăng lên (như LightGCN với 3 lớp). Việc áp dụng Pairwise BPR Loss giúp MF học trực tiếp việc so sánh các cặp phim, phát huy tối đa sức mạnh biểu diễn đặc trưng độc lập của các nhân tử ẩn mà không bị thiên lệch bởi Bias độ phổ biến tuyệt đối.
* Tuy nhiên, LightGCN vẫn là mô hình học sâu đồ thị rất mạnh mẽ khi nắm giữ được cấu trúc láng giềng đa cấp, rất hữu ích cho các tác vụ phức tạp hơn liên quan đến đồ thị đa liên kết.
...
| **Recall@20** | **0.1238** | 0.0833 |
| **NDCG@20** | **0.0486** | 0.0338 |
