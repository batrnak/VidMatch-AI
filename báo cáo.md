...
### 5.1. Kết quả Huấn luyện của Matrix Factorization
Mô hình MF tối ưu hóa RMSE (Root Mean Squared Error) trực tiếp trên điểm số đánh giá. Quá trình hội tụ diễn ra rất mượt mà nhờ việc chuẩn hóa điểm rating dựa trên trung bình điểm của phim (Item-based Normalization).
* **Kết quả cuối cùng:** **Test RMSE = 1.4372** đạt được ở Epoch 30.

![Biểu đồ hội tụ Matrix Factorization](checkpoints/mf_convergence_1m.png)
*Hình 5: Biểu đồ thể hiện sự suy giảm của Loss và chỉ số RMSE trên tập Train/Test.*
...
### 5.3. Trực quan hóa Không gian Nhúng Phim (Movie Embedding Space)
...
*Hình 7: Trực quan hóa không gian nhúng phim bằng t-SNE. Các màu sắc đại diện cho các thể loại phim chính.*
...
2. **Cụm phim Hoạt hình (Animation - Màu tím):** Tạo thành một cụm tách biệt hoàn toàn nằm ở góc trên của đồ thị. Các phim hoạt hình nổi tiếng như *"Toy Story (1995)"*, *"Aladdin (1992)"*, *"Lion King, The (1994)"* nằm sát cạnh nhau. Điều này hợp lý vì nhóm đối tượng xem phim hoạt hình (trẻ em, gia đình) có hành vi xem phim rất tập trung và khác biệt với các nhóm khác.
3. **Cụm phim Hành động (Action - Màu xanh lá) và Viễn tưởng:** Các tác phẩm nổi tiếng như *"Matrix, The (1999)"*, *"Terminator 2: Judgment Day (1991)"*, *"Jurassic Park (1993)"* tụ lại gần nhau, chứng minh mô hình đã học được tính chất hành động/viễn tưởng tương đồng giữa chúng thông qua hành vi của người hâm mộ phim hành động.
4. **Cụm phim Hài (Comedy - Màu xanh lam):** Nằm phân bố rộng rãi hơn ở trung tâm đồ thị, cho thấy tính chất phổ biến và dễ kết hợp của phim hài với các thể loại khác như lãng mạn hay hành động.

---

### 5.4. Trực quan hóa Đồ thị Lưỡng phân User-Item
...
*Hình 8: Trực quan hóa kết nối lưỡng phân giữa User (nút màu cam) và Movie (nút màu xanh dương).*
...
### 5.5. Bảng So sánh và Thảo luận Chuyên sâu

| Tiêu chí so sánh | Matrix Factorization (MF) with Bias | LightGCN (Graph Learning) |
| :--- | :--- | :--- |
| **Loại thuật toán** | Lọc cộng tác cổ điển (Collaborative Filtering) | Học sâu trên đồ thị (Graph Deep Learning) |
| **Đầu ra đầu tiên** | Dự đoán điểm đánh giá cụ thể (Rating prediction) | Dự đoán điểm tương thích xếp hạng (Ranking prediction) |
| **Đặc trưng học được** | Các nhân tử ẩn bậc 1 tuyến tính | Biểu diễn phi tuyến tính đa cấp (láng giềng đa bậc) |
| **Độ đo đánh giá** | RMSE (Root Mean Squared Error) | Recall@K, NDCG@K |
| **Tốc độ huấn luyện** | Cực kỳ nhanh (~50 giây cho 30 epoch trên CPU) | Khá nhanh (~4 phút cho 15 epoch trên GPU CUDA) |
| **Độ phức tạp tính toán**| Thấp, dễ triển khai | Cao hơn, đòi hỏi tài nguyên phần cứng mạnh hơn |
| **Độ tối ưu thực tế** | Thích hợp cho chức năng dự đoán số sao | Thích hợp cho tính năng gợi ý trang chủ (Top-N) |
| **Sai số kiểm định** | **Test RMSE = 1.4372** | **Recall@20 = 0.0833 (NDCG@20 = 0.0338)** |

#### Thảo luận chuyên sâu:
* **Tại sao chỉ số Recall@20 của LightGCN có vẻ thấp (8.33%)?**
  Trong bài toán gợi ý phim thực tế, việc dự đoán chính xác đúng bộ phim tiếp theo mà người dùng sẽ xem (Leave-One-Out) trong số 3,700 bộ phim là cực kỳ khó khăn. Mức Recall@20 đạt **8.33%** trên tập dữ liệu MovieLens 1M là tương đồng với các công bố khoa học chuẩn của thuật toán này khi không kết hợp side information. Nó vượt xa hoàn toàn gợi ý ngẫu nhiên (chỉ đạt tỷ lệ $\frac{20}{3706} \approx 0.54\%$).
* **Hiện tượng Overfitting:** Cả hai mô hình đều không có dấu hiệu Overfitting nghiêm trọng. Đường biểu đồ Loss và RMSE/Recall trên tập Validation bám sát nhau và hội tụ ổn định. Điều này chứng tỏ việc áp dụng Regularization L2 ($\lambda = 0.05$ ở MF và $\lambda = 1e-4$ ở LightGCN) đã hoạt động hiệu quả.
